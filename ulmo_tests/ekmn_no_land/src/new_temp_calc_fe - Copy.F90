    ...
    if(div_m < 0) then
        ...
        if(i==1) then
            ...
            if(j==1) then
                ...
                tan(theta)*(T(h,i+1,j)-T(h,i,calculate_new_lon(j)))/(2*d_theta)  + &
                (1/cos(theta))*(T(h,i,j+1)-2*T(h,i,j)+T(h,i,N_LONS))/(d_phi**2)) + &
                s_ekmn*(dM_theta_dtheta + M(1,i,j)*dT_surf_dtheta-T(h,i,j)*M(1,i,j)*tan(theta)+ &
...
subroutine calc_new_T_deep_ekman(T,M,i,j,h,d_theta,d_phi,s_notrns,s_dfsn,s_ekmn,F_c,F_a,theta,T_new)
