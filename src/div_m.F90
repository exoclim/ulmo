...
subroutine calculate_div_M(lat,lon,mass_flux_phi,mass_flux_theta,div_M)
    ...
    real(real64), dimension(:,:), allocatable :: lats_data_file ! This can be converted to a grid input
    integer(int64) :: col_num
    real(real64), intent(out) :: div_M

    allocate(lats_data_file(N_LATS,2)) ! This can be converted to a grid input

    col_num = 2 ! match latnt64 data type
    lats_data_file = read_file_real(LATS_FILE,N_LATS,col_num) ! This can be converted to a grid input
    lat_value = lats_data_file(lat,2) ! This can be converted to a grid input
    ...
    deallocate(lats_data_file) ! This can be converted to a grid input
